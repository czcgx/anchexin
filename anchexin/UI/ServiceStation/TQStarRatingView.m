...
- (id)initWithFrame:(CGRect)frame numberOfStar:(int)number
{
    ...
    if (self) {
        ...
        self.starBackgroundView = [self buidlStarViewWithImageName:@"star_back"];
        self.starForegroundView = [self buidlStarViewWithImageName:@"star_fore"];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    UITouch *touch = [touches anyObject];
    CGPoint point = [touch locationInView:self];
    TQStarRatingView * weekSelf = self;
...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        imageView.frame = CGRectMake(i *frame.size.width / self.numberOfStar, 0, frame.size.width / self.numberOfStar-10, frame.size.height);
...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point
{
    ...
    int score=(int)(p.x/40) +1;

   // NSLog(@"score::%d",score);
    
    self.starForegroundView.frame = CGRectMake(0, 0, score*40, self.frame.size.height);
