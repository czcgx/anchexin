...
-(void)initUI
{
   ...
   // homeTableView.delegate = self;
    ...
    if (carArray.count>0)
    {
        [carIconImageView setImageWithURL:[NSURL URLWithString:[[carArray objectAtIndex:0] objectForKey:@"iconimg"]] placeholderImage:nil];
    }
    ...
    if (carArray.count>0)
    {
        carIconLabel.text=[[carArray objectAtIndex:0] objectForKey:@"license_number"];//@"沪A38H281";
    }
    ...
    if (carArray.count>0)
    {
        root_healthIndexLabel.text=[[[[carArray objectAtIndex:0] objectForKey:@"healthCheck"] objectForKey:@"totalScore"] stringValue];
    }
    else
    {
    ...
    /*
    //行程分析
    UIImageView *strokeAnalysisImageView=[[UIImageView alloc] initWithFrame:CGRectMake(210, 60, 40, 40)];
    strokeAnalysisImageView.image=IMAGE(@"strokeAnalysis");
    [subView addSubview:strokeAnalysisImageView];
    UIButton *strokeAnalysisButton=[UIButton buttonWithType:UIButtonTypeCustom];
    strokeAnalysisButton.frame=strokeAnalysisImageView.frame;
    [strokeAnalysisButton addTarget:self action:@selector(strokeAnalysis) forControlEvents:UIControlEventTouchUpInside];
    [subView addSubview:strokeAnalysisButton];
    
    //车况诊断
    UIImageView *vehicleDiagnosisImageView=[[UIImageView alloc] initWithFrame:CGRectMake(260, 60, 40, 40)];
    vehicleDiagnosisImageView.image=IMAGE(@"vehicleDiagnosis");
    [subView addSubview:vehicleDiagnosisImageView];
    UIButton *vehicleDiagnosisButton=[UIButton buttonWithType:UIButtonTypeCustom];
    vehicleDiagnosisButton.frame=vehicleDiagnosisImageView.frame;
    [vehicleDiagnosisButton addTarget:self action:@selector(vehicleDiagnosis) forControlEvents:UIControlEventTouchUpInside];
    [subView addSubview:vehicleDiagnosisButton];
    */
...
-(void)customEvent:(UIButton *)sender
{
    ...
    else if (sender.tag==2)
    {
        for (int i=0;i<5; i++)
        {
            int random=arc4random() % (100-[[[[carArray objectAtIndex:0]   objectForKey:@"healthCheck"] objectForKey:@"totalScore"]intValue]);
        ...
        requestTimes=2;
        [[self JsonFactory] get_healthCheck:[[carArray objectAtIndex:0] objectForKey:@"carid"] action:@"healthCheck"];
    }
}
...
-(void)JSONSuccess:(id)responseObject
{
    ...
    else if (responseObject && [[responseObject objectForKey:@"errorcode"] intValue]==0 && requestTimes==1)
    {
       ...
       // NSLog(@"ssssdddd::%@",[[responseObject objectForKey:@"user"] objectForKey:@"userid"]);
        ...
        [APService setTags:nil alias:[NSString stringWithFormat:@"anchexin_%@",[[responseObject objectForKey:@"user"] objectForKey:@"userid"] ] callbackSelector:@selector(tagsAliasCallback:tags:alias:) target:self];
        ...
        if ([[responseObject objectForKey:@"carlist"] count]>0)
        {
            //当前车辆图标
            [carIconImageView setImageWithURL:[NSURL URLWithString:[[[responseObject objectForKey:@"carlist"] objectAtIndex:0] objectForKey:@"iconimg"]] placeholderImage:nil];
          
            //当前车辆车牌号
            carIconLabel.text=[[[responseObject objectForKey:@"carlist"] objectAtIndex:0] objectForKey:@"license_number"];
        }
        
        root_healthIndexLabel.text=[[[[[responseObject objectForKey:@"carlist"] objectAtIndex:0] objectForKey:@"healthCheck"] objectForKey:@"totalScore"] stringValue];//健康指数
    ...
    {
        //保存车辆信息
        [document saveDataToDocument:@"car" fileData:[NSArray arrayWithObject:[responseObject objectForKey:@"car"] ]];
...
-(void)initWeatherUI
{
    ...
    UILabel *upWeatherLabel=[[UILabel alloc] initWithFrame:CGRectMake(80, 25, 60, 20)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag==2)
    {
        ...
        if ([[[carArray objectAtIndex:0] objectForKey:@"license_number"] isEqualToString:[[carListArray objectAtIndex:indexPath.row] objectForKey:@"licenseNumber"]])
        {
