...
@interface SinoNetMBProgressHUD ()
...
@implementation SinoNetMBProgressHUD
...
static SinoNetMBProgressHUD *_sinoNetMbProgresshud;
 

-(SinoNetMBProgressHUD*)shareMBProgressHUD{
    if (!_sinoNetMbProgresshud) {
        _sinoNetMbProgresshud = [[SinoNetMBProgressHUD alloc] init];
    }
    return _sinoNetMbProgresshud;
}

- (void)setMode:(SinoNetMBProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == SinoNetMBProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == SinoNetMBProgressHUDModeCustomView && self.customView != nil){
...
+ (SinoNetMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	SinoNetMBProgressHUD *hud = [[SinoNetMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ...
    if (view) {
        ...
        for (UIView *v in [view subviews]) {
            if ([v isKindOfClass:[SinoNetMBProgressHUD class]]) {
                viewToRemove = v;
            }
        }
        if (viewToRemove != nil) {
            SinoNetMBProgressHUD *HUD = (SinoNetMBProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"SinoNetMBProgressHUDViewIsNillException" 
					format:@"The view used in the SinoNetMBProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = SinoNetMBProgressHUDAnimationFade;
        self.mode = SinoNetMBProgressHUDModeIndeterminate;
        self.labelText = nil;
...
//    if ([ self viewWithTag:911]==nil) {
//        UIImage *img = [UIImage imageNamed:@"银商加载图片.png"];
//        UIImageView *imageV = [[UIImageView alloc] initWithImage:img];
//        imageV.frame = indicator.frame;
//        
//        CGRect r = indicator.frame;
//        r.size = img.size;
//        
//        r.origin = CGPointMake(indicator.frame.origin.x-90, indicator.frame.origin.y-55);
//        imageV.frame = r;
//        imageV.tag = 911;
...
//    }
    ...
    if (nil != self.labelText) {
        ...
        CGSize dims= [self.labelText sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:LABELFONTSIZE]}];
...
//            UIImage *img = [UIImage imageNamed:@"银商加载图片.png"];
//            UIImageView *imageV = [[UIImageView alloc] initWithImage:img];
//            imageV.frame = label.frame;
//            
//            CGRect r = label.frame;
//            r.size = img.size;
//            
//            r.origin = CGPointMake(label.frame.origin.x-80, label.frame.origin.y-80);
//            imageV.frame = r;
//            imageV.tag = 911;
//            [self addSubview:imageV];
//            [imageV release];
...
//        
        ...
        if (nil != self.detailsLabelText) {
           ...
           // detailsLabel.backgroundColor = [UIColor clearColor];
            ...
            CGSize constraint =CGSizeMake(frame.size.width - 4*margin, maxHeight);
            NSDictionary *attribute = @{NSFontAttributeName: [UIFont systemFontOfSize:16.0]};
            CGSize labelSize = [detailsLabel.text boundingRectWithSize:constraint options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
...
//-(void)showMainWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated {
//    
//    methodForExecution = method;
//    targetForExecution = [target retain];
//    objectForExecution = [object retain];
...
//    // Launch execution in new thread
//	taskInProgress = YES;
//    [self performSelectorOnMainThread:@selector(launchExecution ) withObject:nil waitUntilDone:YES];
//    [NSThread detachNewThreadSelector:@selector(launchExecution) toTarget:self withObject:nil];
//	
//	// Show HUD view
//	[self show:animated];
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == SinoNetMBProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == SinoNetMBProgressHUDAnimationZoom) {
